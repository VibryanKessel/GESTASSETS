...
<body>
	...
	<form method="post">
		...
		<input type="search" title="Votre recherche" autocomplete="off" placeholder="Entrer un type d'asset" class="page-link" id="search" onkeyup="document.getElementById('select').selectedIndex=0;">
        <div class="loupe page-link"><label for="search"><i class="fa fa-search display-5" aria-hidden="true"></i></label></div>
        <select required id="select" class="bg-secondary" onclick="document.getElementById('search').value=this.options[this.selectedIndex].text;">
        </select>
		</div>
	</form>
